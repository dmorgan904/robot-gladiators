var fightOrSkip = function() {
  //ask player if they'f like to fight or skip using fightOrSkip function
...
var fight = function(enemy) {
  ...
  while (enemy.health > 0 && playerInfo.health > 0) {
   ...
   enemy.health = Math.max(0, enemy.health - damage);
    ...
    var damage = randomNumber(0, playerInfo.health - damage);
...
var startGame = function() {
    ...
    for(var i= 0; i < enemyInfo.length; i++) {
      ...
      if (playerInfo.health > 0) {
        ...
        if (playerInfo.health > 0 && i < enemyInfo.length - 1) {
          // aks if player wants to use the store before next round
    ...
    //after the loop ends, plyaer is either out of health or enemies to fight, so run the endGame function
      endGame();
